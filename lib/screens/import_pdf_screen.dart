...
class _ImportPdfScreenState extends State<ImportPdfScreen> {
    ...
    try {
      ...
      if (result != null) {
        ...
        if (scores.isEmpty) {
          ScaffoldMessenger.of(
            context,
          ).showSnackBar(const SnackBar(content: LText('未能在PDF中找到有效的成绩数据')));
        } else {
          // 成功解析，返回数据
  ...
  List<Score> _parseTranscriptTextStream(String text) {
    ...
    final dataBlockPattern = RegExp(
      r'(\d+(?:\.\d+)?)\s+(\d+)\s+([A-Z][+-]?|\d+(?:\.\d+)?)\s+(\d+(?:\.\d+)?)',
    );
    ...
    for (int i = 0; i < allMatches.length; i++) {
      ...
      if (courseName.isNotEmpty) {
        String semesterName = _findSemesterForColumn(
          text,
          match.start,
          currentSemesterIndex % columnCount,
          detectedSemesters,
          columnCount,
        );
  ...
  String _findSemesterForColumn(
    ...
    if (matches.isEmpty) {
      return allSemesters.isNotEmpty
          ? allSemesters[columnIndex % allSemesters.length]
          : "未知学期";
    }
